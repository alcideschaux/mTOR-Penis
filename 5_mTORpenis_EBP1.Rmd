...
```{r 4EBP1_Levels}
marker.cut <- quantile(EBP1, probs = 0.66, na.rm = TRUE)
marker <- ifelse(EBP1 > marker.cut, c("High phos-4EBP1"), c("Low phos-4EBP1"))
marker <- factor(marker, levels = c("Low phos-4EBP1", "High phos-4EBP1"))
```
***

#### Immunohistochemical expression by H-scores

```{r 4EBP1_DescriptiveH}
var <- EBP1
t <- "H-score"
...
#### Tumor progression
```{r 4EBP1_AssociationProgression}
outcome <- progression
t <- "Tumor Progression"
plot.categorical.group(marker, outcome, align = "v", xlab = t)
descriptive.categorical.group(marker, outcome)
...
#### Distant metastasis
```{r 4EBP1_AssociationDistantMetastasis}
outcome <- mets
...
#### Overall mortality
```{r 4EBP1_AssociationOverallMortality}
outcome <- dead
t <- "Overall Mortality"
plot.categorical.group(marker, outcome, align = "v", xlab = t)
descriptive.categorical.group(marker, outcome)
...
```{r 4EBP1_AssociationCancerMortality}
...
```{r 4EBP1_SurvivalRecurrence, fig.width = 10, fig.height = 7}
# Tumor recurrence
outcome <- recurrence
...
```{r 4EBP1_SurvivalProgression, fig.width = 10, fig.height = 7}
# Tumor progression
outcome <- progression
...
```{r 4EBP1_SurvivalDistantMetastasis, fig.width = 10, fig.height = 7}
# Distant metastasis
outcome <- mets
fu <- date.met.mo
t <- "Distant Metastasis"
...
```{r 4EBP1_SurvivalOverallMortality, fig.width = 10, fig.height = 7}
# Overall mortality
outcome <- dead
fu <- fu.last.mo
t <- "Overall Mortality"
...
```{r 4EBP1_SurvivalCancerMortality, fig.width = 10, fig.height = 7}
# Cancer-related mortality
outcome <- died.disease
fu <- fu.last.mo
t <- "Cancer-Related Mortality"
